...
public class HourlyEmployee extends Employee {
    ...
    public HourlyEmployee(String name, LocalDate hireDate, double rate, double hours) throws
            IllegalWageException{
    ...
    public void setRate(double rate) throws IllegalWageException {
        ...
        try {
            if(rate<FEDERAL_MINIMUM_WAGE){
                throw new IllegalWageException(getName()+ "'s hourly wage "+rate
                        +" is below federal minimum wage "+FEDERAL_MINIMUM_WAGE+".");
            }else{
                this.rate = rate;
            }
        } catch (IllegalWageException e) {
            System.out.println(getName()+ "'s wage is illegal: "+getRate());
        }
